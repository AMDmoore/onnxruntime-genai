...
void EnsureDeviceOrtInit(DeviceInterface& device, const Config& config) {
  ...
  // CreateMemoryInfo_V2 in onnx-hipdnn-ep/3rd-party/morphizen/.../morphizen-ep-factory.cpp
  // GetSupportedDevicesImpl when registering the GPU OrtMemoryInfo with
  // EpDevice_AddAllocatorInfo; otherwise OGA cannot look up the EP's
  // hipMalloc-based allocator.
  ...
  try {
    ...
    if (type == DeviceType::MorphiZenEP) {
      ...
      // (CreateMemoryInfo_V2 in morphizen-ep-factory.cpp's
      // GetSupportedDevicesImpl, registered via EpDevice_AddAllocatorInfo).
      // The legacy OrtMemoryInfo::Create only knows ORT-internal device names
      // ("Cuda", "DML", ...), so it would fail with
      // "Specified device is not supported. Try CreateMemoryInfo_V2."
      // We must use CreateMemoryInfo_V2 with the same parameters morphizen
      // used when registering its OrtMemoryInfo so ORT can match the EP's
      // hipMalloc-based allocator.

...
// Mirrors ryzenai/interface.cpp. The only behavioural differences are:
//   - ep_name_ matches the registration_name passed to morphizen's
//     CreateEpFactories (== "MorphiZenEP" — what callers like model_benchmark
//     use with --ep_library, what genai_config.json uses as provider name,
//     and what the OGA fork's session_options dispatch table keys on)
//   - SetupProvider filters OrtHardwareDeviceType_GPU only; ep_name match is
//     already a strong enough filter (the factory itself rejects non-AMD-GPU
//     hardware in GetSupportedDevicesImpl)
//   - DeviceType::MorphiZenEP is returned from GetType()
//   - Allocator-backed memory is treated as both host- and device-accessible
//     (true on AMD APU iGPU; same simplification RyzenAI uses)
...
// Distinct from 0x1022, which is the AuthenticAMD CPU/NPU vendor id.
static constexpr uint32_t kAmdGpuVendorId = 0x1002;
...
struct Interface : MorphiZenEPInterface {
  ...
  ~Interface() {
    // No equivalent of RyzenAI_Shutdown for morphizen yet; the EP DLL
    // releases its singletons in DllMain on unload.
  }
  ...
  void SetupProvider(OrtSessionOptions& session_options, const ProviderOptions& provider_options) override {
...

    {
      ...
      for (const auto& device : std::span{devices, ndevices}) {
        ...
        // itself to AMD GPU vendor id (kAmdGpuVendorId, 0x1002), so any
        // OrtEpDevice carrying our ep_name is by construction AMD GPU. We do
        // not re-check the vendor id here to keep the filter forgiving (the
        // factory could in future expand to other AMD-vendor GPU variants
        // without us needing to update this list in lockstep).
